...
class LatentSkipConnection(nn.Module):
    r"""Pluggable latent skip connection for decoder.

    Injects latent information at each decoder stage via bilinear interpolation
    and a learned 1x1 Conv projection. Can be swapped for other skip strategies.

    Arguments:
        lat_channels: Number of latent channels (e.g. 8 for x4).
        target_channels_list: List of target channel counts at each decoder stage.
            For hid_channels=(64,128,256), reversed decoder order is [256, 128, 64].
        target_sizes: List of (H, W) spatial sizes at each decoder stage.
            For 16→32→64: [(16,16), (32,32), (64,64)].
        spatial: Number of spatial dimensions.
        mode: Interpolation mode ('bilinear', 'nearest', etc.).
        periodic: Whether to use circular padding.
    """

    def __init__(
        self,
        lat_channels: int,
        target_channels_list: Sequence[int],
        ...
        self.mode = mode
        self.target_sizes = [tuple(s) for s in target_sizes]

        padding_mode = "circular" if periodic else "zeros"

        # One 1x1 Conv per stage to map lat_channels → target_channels
        self.projections = nn.ModuleList()
        for tch in target_channels_list:
            self.projections.append(
                ConvNd(
                    lat_channels, tch,
                    spatial=spatial,
                    kernel_size=tuple([1] * spatial),
                    padding=tuple([0] * spatial),
                    padding_mode=padding_mode,
                )
            )

        # Initialize small so skip starts near-zero (stable training)
        for proj in self.projections:
            proj.weight.data.mul_(1e-2)

    def forward(self, z: Tensor, stage_idx: int) -> Tensor:
        r"""Compute skip value for a given decoder stage.

        Arguments:
            z: The latent tensor, with shape (B, lat_channels, H_z, W_z).
            stage_idx: Which decoder stage (0 = deepest, 1 = middle, 2 = shallowest).

        Returns:
            Skip tensor with shape matching the target stage.
        """
        target_size = self.target_sizes[stage_idx]
        z_size = tuple(z.shape[2:])

        if z_size != target_size:
            z_up = nn.functional.interpolate(
                z, size=target_size, mode=self.mode,
                align_corners=False if self.mode == "bilinear" else None,
            )
        else:
            z_up = z

        return self.projections[stage_idx](z_up)
...
class DCDecoder(nn.Module):
    ...
    def forward(self, x: Tensor, latent_skip: Optional['LatentSkipConnection'] = None) -> Tensor:
        r"""
        Arguments:
            x: The input tensor, with shape :math:`(B, C_i, L_1, ..., L_N)`.
            latent_skip: Optional LatentSkipConnection module to inject latent info.

        Returns:
            The output tensor, with shape :math:`(B, C_o, L_1 \times 2^D, ..., L_N  \times 2^D)`.
        """

        z = x  # save latent for skip connections
        skip_injected = False

        for stage_idx, blocks in enumerate(self.ascent):
            skip_injected = False
            for block in blocks:
                # Inject skip right before the first ResBlock in each stage
                if latent_skip is not None and not skip_injected and isinstance(block, ResBlock):
                    x = x + latent_skip(z, stage_idx)
                    skip_injected = True
                x = block(x)
